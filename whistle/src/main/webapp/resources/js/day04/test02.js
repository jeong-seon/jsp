...
//	document.getElementById('tid').value = null;
	...
	if(sid) {
		// 변수에 데이터가 채워져 있는 경우(null 또는 undefined 가 아닌 경우)
		document.getElementById('tpw').focus();
	} else {
		// sid 에 데이터가 채워져 있지 않은 경우(null 또는 undefined 인 경우)
...
// 통과하지 않은 경우는 입력 내용을 모두 지우고 다시 입력하도록 하세요.

// 비밀번호는 첫 문자는 알파벳이어야 하고,
...
//		location.href = 'test.html?id=' + sid + '&pw=' + spw;
		// 데이터 전송용 form 태그의 input 태그에 읽은 데이터를 입력해서
		// 전송하세요.
