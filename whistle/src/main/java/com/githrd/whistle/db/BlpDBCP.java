...
public class BlpDBCP {
	...
	public BlpDBCP() {
		try {
			...
			/*
				...
				==> 이 작업이 무사히 완료되면
					커넥션 풀을 찾아서 기억(ds 변수)해놓았다는 것이 된다.
			 */
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void close(Object o) {
		/*
			이 함수를 호출할 때 어떤 자원을 입력해서 호출할 지 알 수 없으므로
			모든 타입을 받을 수 있는 Object 타입으로 받기로 한다.
			
			그리고 입력된 데이터의 타입을 확인한 후
			확인된 형태로 강제 형변환 한 후 닫으면 된다.ㄴ
		 */
		try {
