 ...
 *	요청 내용을 분석하고 실행할 클래스를 선택해서 실행(디스패치)시켜줄 서블릿 클래스이다.
 * @author	정선우
 * @since	2022/05/11
 * @version	v.1.0
 * @see
...
// 이 서블릿이 언제 실행될지 url-pattern 을 지정하는 부분
@WebServlet("*.blp")
public class BlpDispatch extends HttpServlet {
/*
	
 */
	private HashMap<String, BlpInter> map;	// 실제 요청내용과 실제 실행할 객체를 입력할 맵.
...
//			System.out.println("### Dispatch Path : " + path);
		...
		// properties 에 기억된 클래스의 경로를 이용해서 Map 을 완성한다.
		map = new HashMap<String, BlpInter>();
		// 키값만 뽑아오고
		Enumeration en = prop.keys();
		// 키값에 해당하는 클래스 객체를 만들어서 map 에 채운다.
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
			이 때
				isRedirect 의 속성값이 false 인 경우는 forward 로 처리하고
							  속성값이 true 인 경우는 redirect 로 처리하고
							  속성값이 null 인 경우는 비동기통신으로 처리하면 된다.
		 */
...
//		System.out.println("# service real : " + real);
		
		// 원하는 컨트롤러를 선택해서
		// 위에서 map 에 등록된 것을 이용해서 꺼내면 된다.
		...
		if(bool == null) {
			// 이 경우는 비동기 통신(ajax)이 처리되어야 한다.
			PrintWriter pw = resp.getWriter();
			pw.print(view);
		} else if(bool) {
			// redirect 작업으로 처리해야되는 경우
